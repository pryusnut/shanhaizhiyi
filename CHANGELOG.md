...
- **我方登场人数新增"四人挑战"选项与"登场人数全局覆盖"开关**："我方登场人数"设置新增第 4 档"四人挑战"（选将数 [4,4]）；其下方新增开关"登场人数全局覆盖"（视觉与功能同"朱果获取全局覆盖"：关闭时仅影响扩展外关卡、12 个扩展关卡走各自预设；打开时覆盖所有关卡；intro 暂为占位文本"人数开关"待完善）。实现：预设表 12 关新增 `renshu` 字段（占位默认三人挑战），新增 `shzy_renshu()` 解析函数，选将人数读取点统一改为 `shzy_renshu()`。
